...
**The colors represent a heterogeneous aspect of our sample.** That's just a fancy way of saying, "It varies." Think of this as being something like household income or race or age group. In an RCT, there is all sorts of "unobserved heterogeneity": underlying aspects of the experimental groups that may not be measurable. This data set will be used in the subsequent simulations.
...
The table below reflects the first few observations from the underlying data set (starting from the bottom left). Y<sub>0</sub> and Y<sub>1</sub> are potential outcomes: we're operating in a world where we know exactly what outcome we would see whether each observation received or did not receive treatment. These are affected by the Treatment Effect and the Outcome Predictiveness of Color selected at left.
...
output$observed_effect <- renderText({
  ...
  msg <- paste("The observed effect is", obs_effect %>% percent(accuracy = 0.1))
})
